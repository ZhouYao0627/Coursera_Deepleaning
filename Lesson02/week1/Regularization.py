...
from Lesson02.week1 import reg_utils
import sklearn
import sklearn.datasets
import scipy.io
from Lesson02.week1.testCases import *
...
1.不使用正则化
2.使用正则化
  2.1 使用L2正则化:将lambd输入设置为非零值。 我们使用“lambd”而不是“lambda”，因为“lambda”是Python中的保留关键字。
  2.2 使用随机节点删除(dropout):将keep_prob设置为小于1的值
"""
...
# 因为改变了成本函数，我们也必须改变向后传播的函数， 所有的梯度都必须根据这个新的成本值来计算。
...
# 下面我们将关闭第一层和第三层的一些节点，我们需要做以下四步:详见https://blog.csdn.net/u013733326/article/details/79847918
def forward_propagation_with_dropout(X, parameters, keep_prob=0.5):
    ...
    import numpy as np
    np.random.seed(1)
    A1 = np.random.randn(1,3)  # [[ 1.62434536 -0.61175641 -0.52817175]]

    D1 = np.random.rand(A1.shape[0],A1.shape[1])  # [[0.39676747 0.53881673 0.41919451]]
    keep_prob=0.5
    D1 = D1 < keep_prob  # [[ True False  True]]

    A1 = 0.01
    A1 = A1 * D1  # [[0.01 0.   0.01]]
    A1 = A1 / keep_prob  # [[0.02 0.   0.02]]
    """
...
# 改变了前向传播的算法，我们也需要改变后向传播的算法，使用存储在缓存中的掩码D^[1]和D^[2]将舍弃的节点位置信息添加到第一个和第二个隐藏层。
...
parameters = model(train_X, train_Y, is_plot=True)  # 不使用正则化
parameters = model(train_X, train_Y, lambd=0.7, is_plot=True)  # 使用正则化
parameters = model(train_X, train_Y, keep_prob=0.86, learning_rate=0.3, is_plot=True)  # 使用随机删除节点
